...
function prepare_evaluation {
	if [ "x" != "x${EVAL_EXPRESSION}" ] || [ "x" != "x${EVAL_EXPRESSION_FROM_FILE}" ]; then
		...
		if [ "x" == "x${PYTHON_VAR}" ]; then
			PYTHON_VAR=${PYTHON_DEFAULT_VAR};
		fi;
		if [ "x" != "x${EVAL_EXPRESSION}" ]; then
			tmp=$( echo ${EVAL_EXPRESSION} | grep -w ${PYTHON_VAR} &> /dev/null );
			if [ $? -eq 0 ]; then
				PYTHON_EXPRESSION="${EVAL_EXPRESSION}\nprint(${PYTHON_VAR})";
			else
				PYTHON_EXPRESSION="${PYTHON_VAR}=${EVAL_EXPRESSION}\nprint(${PYTHON_VAR})";
			fi;
			PYTHON_CMD=$( printf "${PYTHON_VARS_REF}\n${PYTHON_VARS_USR}\n${PYTHON_EXPRESSION}");
		else
			tmp=$( echo ${EVAL_EXPRESSION_FROM_FILE} | grep -w ${PYTHON_VAR} &> /dev/null );
			if [ $? -eq 0 ]; then
				PYTHON_EXPRESSION="${EVAL_EXPRESSION_FROM_FILE}\nprint(${PYTHON_VAR})";
			else
				PYTHON_EXPRESSION="${PYTHON_VAR}=${EVAL_EXPRESSION_FROM_FILE}\nprint(${PYTHON_VAR})";
			fi;
			PYTHON_CMD=$( printf "${PYTHON_VARS_REF}\n${PYTHON_VARS_USR}\n${PYTHON_EXPRESSION}");
		fi;
...
VARIABLESP_LIST="";
PBSVARSP_LIST="";
PYTHON_DEFAULT_VAR="__RESULT";
PYTHON_EXPR_FILE="";
PYTHON_VAR="";
EVAL_EXPRESSION="";
EVAL_EXPRESSION_FROM_FILE="";
...
ERROR_CODE_PYTHONEXPRFILE=8;
ERROR_CODE_PYTHONVAR=9;
ERROR_CODE_VARPREFIXES=10;
ERROR_CODE_EXECUTION=11;
...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    -h|--help)
      ...
      echo " -evaluation_expr EXPR             : Python expression for the evaluation";
      echo " -evaluation_file FILE             : Python file with the expression for the evaluation";
      echo " -evaluation_var VAR               : Python variable with the score";
    ...
    -evaluation_var)
      if [[ $# -gt 1 ]]; then
        PYTHON_VAR="$2";
        shift
		if [ "x${PYTHON_VAR}" == "x" ]; then
			JSON_ERROR_CODE=${ERROR_CODE_PYTHONVAR};
			JSON_ERROR_MSG="Python variable is empty!";
			exit_script
		fi;	
      fi;  
    ;;
...
## Determine if python file expression exist
if [ "x${PYTHON_EXPR_FILE}" != "x" ]; then
	if [ ! -d ${PYTHON_EXPR_FILE} ]; then
		EVAL_EXPRESSION="";
		EVAL_EXPRESSION_FROM_FILE=$( cat ${PYTHON_EXPR_FILE} 2> /dev/null; );
	else
		JSON_ERROR_CODE=${ERROR_CODE_PYTHONEXPRFILE};
		JSON_ERROR_MSG="Python expression file does not exist!";
		exit_script
	fi;	
fi;	

...
function write_json {
	...
	## Execute json 
	JSON_OUTPUT=$( echo ${OUTPUT_TEXT} | python -c 'import json,sys;from collections import OrderedDict; print json.dumps(json.load(sys.stdin, object_pairs_hook=OrderedDict), indent=4);');
	printf "${JSON_OUTPUT}\n"
}
...
## FUNCTION READ_PBS_REF_OUTERR_FILENAME - BEGIN

function read_pbs_ref_outerr_filename {
	outerr_filename=$( cat $1 2> /dev/null );
	if [ $? -ne 0 ]; then
		JSON_ERROR_CODE=${ERROR_CODE_FILE_NOT_READABLE};
		JSON_ERROR_MSG="File $1 cannot be read! It is not possible to determine file names for output/error files in reference output folder!";
		exit_script
	fi;
}

## FUNCTION READ_PBS_REF_OUTERR_FILENAME - END

## FUNCTION WRITE_PBS_REF_OUTERR_FILENAME - BEGIN

function write_pbs_ref_outerr_filename {
	echo $2 > $1;
	if [ $? -ne 0 ]; then
		JSON_ERROR_CODE=${ERROR_CODE_FILE_NOT_WRITABLE};
		JSON_ERROR_MSG="File $1 cannot be written! It was not possible to store the file names for output/error files in reference output folder!";
		exit_script
	fi;
}

## FUNCTION WRITE_PBS_REF_OUTERR_FILENAME - END
...
ERROR_CODE_PBS_ID_NOT_SPECIFIED=2;
ERROR_CODE_PBS_ID_QSTAT_FAILED=3;
ERROR_CODE_OUT_FOLDER_NOT_SPECIFIED=4;
ERROR_CODE_OUT_FOLDER_NOT_FOUND=5;
ERROR_CODE_FILE_NOT_FOUND=6;
ERROR_CODE_FILE_NOT_READABLE=7;
ERROR_CODE_FILE_NOT_WRITABLE=8;
...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    -h|--help)
      ...
      echo " -pbsid PBS_ID                     : PBS ID of the job (e.g. -pbsid 3010)";
...
## PBS ID validation and get qstat output (qstat_exec_info)
if [[ "x" == "x${PBS_ID}" ]]; then
	JSON_ERROR_CODE="${ERROR_CODE_PBS_ID_NOT_SPECIFIED}";
	JSON_ERROR_MSG="PBS ID was not specified!";
	exit_script
else
	qstat_exec_info=$( qstat -xf ${PBS_ID} 2>&1 );
	if [ $? -ne 0 ]; then
		JSON_ERROR_CODE="${ERROR_CODE_PBS_ID_QSTAT_FAILED}";
		JSON_ERROR_MSG="qstat command failed: ${qstat_exec_info}!";
		exit_script
	fi;
	job_state=$( printf "${qstat_exec_info}\n" | grep -i "job_state" | awk -v FS=" = " '{print $2}' );
	if [[ "F" != "${job_state}" ]]; then
		JSON_ERROR_CODE="${ERROR_CODE_PBS_ID_QSTAT_FAILED}";
		JSON_ERROR_MSG="qstat: job_state is not F: $( printf "${qstat_exec_info}\n" | grep -i "job_state")!";
		exit_script
	fi;	
fi;
...
## Determine if the output and error files exist for reference
pbs_ref_outerr_filename="pbs_ref_outerr_filename";
if [ ${CLI_ONLYREF} -eq 1 ]; then
	outerr_filename=${PBS_ID}.$( hostname );
	write_pbs_ref_outerr_filename ${REF_OUT_FOLDER_NAME}/${pbs_ref_outerr_filename} ${outerr_filename};
else
	read_pbs_ref_outerr_filename ${REF_OUT_FOLDER_NAME}/${pbs_ref_outerr_filename};
fi;	
file_to_check=${REF_OUT_FOLDER_NAME}/${outerr_filename}.OU;
check_file_exist ${file_to_check}
ref_output_file=${file_to_check};
file_to_check=${REF_OUT_FOLDER_NAME}/${outerr_filename}.ER;
check_file_exist ${file_to_check}
ref_error_file=${file_to_check};

## Read or write the qstat information for reference
pbs_ref_qstat_file=${REF_OUT_FOLDER_NAME}/${outerr_filename}.QSTAT;
if [ ${CLI_ONLYREF} -eq 1 ]; then
	printf "${qstat_exec_info}\n" > ${pbs_ref_qstat_file} 2> /dev/null;
	if [ $? -ne 0 ]; then
		JSON_ERROR_CODE=${ERROR_CODE_FILE_NOT_WRITABLE};
		JSON_ERROR_MSG="File $1 cannot be written! It was not possible to store the qstat info in reference output folder!";
		exit_script
	fi;
	ref_qstat_exec_info=${qstat_exec_info};
else
...
## Determine if the output and error files exist for user
if [ ${CLI_ONLYREF} -eq 0 ]; then
	file_to_check=${USR_OUT_FOLDER_NAME}/${PBS_ID}.$( hostname ).OU;
	check_file_exist ${file_to_check}
	usr_output_file=${file_to_check};
	file_to_check=${USR_OUT_FOLDER_NAME}/${PBS_ID}.$( hostname ).ER;
	check_file_exist ${file_to_check}
...
## Retrieve pbs_id from cli
JSON_TESTEXEC_PBS_ID=${PBS_ID};
...
if [ ${PBSVARSP_LIST_NUM} -gt 0 ]; then
	...
	for ivar in ${!PBSVARSP_LIST_ARRAY[@]}; do
		...
		if [ "x" != "x${ref_text}" ]; then
			...
			if [ ${CLI_ONLYREF} -eq 0 ]; then
				usr_output_text=$( printf "${qstat_exec_info}\n" | sed -n -E "/${filter}/p" );
